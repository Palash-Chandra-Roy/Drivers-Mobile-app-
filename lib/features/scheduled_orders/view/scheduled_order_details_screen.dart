...
class ScheduledOrderDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final order = context.watch<ScheduledOrderProvider>().selectedOrder;
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(order.id, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),
              const SizedBox(height: AppSizes.paddingMd),
              _DetailRow(Icons.calendar_today, 'Scheduled', DateFormatter.formatDateTime(order.scheduledDate)),
              ...
              _DetailRow(Icons.attach_money, 'Earning', AppHelpers.formatCurrency(order.price)),
              const Spacer(),
              CustomButton(
                title: 'Start Order',
                onPressed: () {
                  AppHelpers.showSnackBar(context, 'Order started!');
                  Navigator.pushNamed(context, RouteNames.acceptOrder);
                },
              ),
            ],
          ),
